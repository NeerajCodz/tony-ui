...
>(({ className, ...props }, ref) => (
  <TableHeaderBase ref={ref} className={cn('[&_tr]:border-b border-[var(--ac-border)]', className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <TableRowBase
    ref={ref}
    className={cn(
      'border-b border-[var(--ac-border)] transition-colors hover:bg-[var(--ac-accent)]/5 data-[state=selected]:bg-[var(--ac-accent)]/10',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <TableHeadBase
    ref={ref}
    className={cn(
      'h-12 px-4 text-left align-middle font-bold text-[var(--text-muted)] [&:has([role=checkbox])]:pr-0 font-mono uppercase tracking-wider',
      className
    )}
    {...props}
  />
