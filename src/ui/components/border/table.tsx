...
import { TableBase, TableHeaderBase, TableBodyBase, TableFooterBase, TableRowBase, TableHeadBase, TableCellBase, TableCaptionBase } from '@/ui/components/_base/table';
import { cn } from '@/lib/utils';

const Table = React.forwardRef<HTMLTableElement, React.ComponentPropsWithoutRef<typeof TableBase>>(
  ({ className, ...props }, ref) => (
    <div className="relative w-full overflow-auto rounded-none border border-[var(--br-border-dim)]">
      <TableBase
        ref={ref}
...
const TableHeader = React.forwardRef<HTMLTableSectionElement, React.ComponentPropsWithoutRef<typeof TableHeaderBase>>(
  ({ className, ...props }, ref) => (
    <TableHeaderBase ref={ref} className={cn('[&_tr]:border-b bg-muted', className)} {...props} />
  )
);
