...
const AnalogClock = React.forwardRef<SVGSVGElement, AnalogClockProps>(
  ({ className, effects = 'on', size = 'md', visualType = 'outline', ...props }, ref) => {
    ...
    return (
      <svg
        ref={ref}
        className={cn(tacticalHudEffectsClass(effects), 'bg-[var(--th-surface)] text-[var(--text-primary)] border border-[var(--th-hex-line)] rounded-full', sizeClasses[size], className)}
        viewBox="0 0 100 100"
        {...props}
      >
        <circle cx="50" cy="50" r="48" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--th-hex-line)]" />
        ...
        <line
          ...
          className="text-[var(--th-plasma-1)]"
        />
        
        <circle cx="50" cy="50" r="3" fill="currentColor" className="text-[var(--th-plasma-1)]" />
      </svg>
    );
  }
);
