...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      border: `2px solid hsl(var(--${color}-base))`, boxShadow: `0 0 10px hsl(var(--${color}-base)), inset 0 0 5px hsl(var(--${color}-base))`, background: 'transparent'
    , ...typeStyles}}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             borderBottom: `2px solid hsl(var(--${color}-base))`, background: `hsl(var(--${color}-base) / 0.1)`
          , ...typeStyles}}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: `1px solid hsl(var(--${color}-base) / 0.5)` , ...typeStyles}}
                >
                  {columns.map((col, j) => (
