...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      border: `1px solid hsl(var(--${color}-base) / 0.5)`, background: `hsl(var(--${color}-base) / 0.05)`, clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)'
    }}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             background: `hsl(var(--${color}-base) / 0.1)`, borderBottom: `1px solid hsl(var(--${color}-base) / 0.3)`
          }}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: `1px solid hsl(var(--${color}-base) / 0.1)` }}
                >
                  {columns.map((col, j) => (
