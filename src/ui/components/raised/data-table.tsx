...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)', border: 'none', borderRadius: '0.5rem'
    , ...typeStyles}}>
      <div 
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: '1px solid hsl(var(--border))' , ...typeStyles}}
                >
                  {columns.map((col, j) => (
