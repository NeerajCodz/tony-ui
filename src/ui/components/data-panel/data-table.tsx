...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      borderTop: `2px solid hsl(var(--${color}-base))`, borderBottom: `2px solid hsl(var(--${color}-base))`, background: 'hsl(var(--background))'
    , ...typeStyles}}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             background: 'hsl(var(--muted))', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.75rem'
          , ...typeStyles}}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: '1px dashed hsl(var(--border))' , ...typeStyles}}
                >
                  {columns.map((col, j) => (
