...
const typesDir = path.join(__dirname, '../src/ui/types/components');
...
export const Tabs = ({ 
  ...
  const Component = useMemo(() => lazy(() => import(\`./tabs-\${version}\`).then(module => ({ default: module.TabsRoot }))), [version]);
...
const createSubComponent = (name: string, exportName: string) => {
  return ({ children, ...props }: any) => {
    ...
    const Component = useMemo(() => lazy(() => import(\`./tabs-\${version}\`).then(module => ({ default: module[exportName] }))), [version]);
...
`;
  ...
  fs.writeFileSync(path.join(componentDir, `tabs-${version}.tsx`), content);
  console.log(`Generated tabs-${version}.tsx`);
};

// Execute
generateIndex();
versions.forEach(v => generateVersion(v));
console.log('Tabs regeneration complete.');
