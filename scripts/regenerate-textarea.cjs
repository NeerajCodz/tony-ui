...
const COMPONENT_PATH = path.join(UI_PATH, 'components/textarea');

if (!fs.existsSync(COMPONENT_PATH)) {
  fs.mkdirSync(COMPONENT_PATH, { recursive: true });
}

// Textarea Clip Paths - Similar to Inputs but taller
const VERSIONS = {
  'angular-corner': {
      ...
      >
        ...
        <div className="relative z-10 p-3">
          <textarea
            ref={ref}
            rows={rows}
            className="w-full bg-transparent border-none outline-none text-sm text-white placeholder-white/30 font-mono focus:ring-0 p-0 m-0 resize-none"
            disabled={disabled}
            {...props}
          />
        </div>
...
Object.entries(VERSIONS).forEach(([name, config]) => {
  ...
  const filePath = path.join(COMPONENT_PATH, `textarea-${name}.tsx`);
  fs.writeFileSync(filePath, content);
  console.log(`Generated textarea-${name}.tsx`);
});
