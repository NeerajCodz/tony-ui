  ...
  ({ className, variant, ...props }, ref) => {
    return (
      <ToastBase
        ...
        variant={variant}
        className={cn(
          'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-none border border-[var(--br-border-dim)] bg-[var(--br-bg)] p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
...
const ToastAction = React.forwardRef<React.ComponentRef<typeof ToastActionBase>, React.ComponentPropsWithoutRef<typeof ToastActionBase>>(
  ({ className, ...props }, ref) => (
    <ToastActionBase
      ref={ref}
      className={cn(
        'inline-flex h-8 shrink-0 items-center justify-center rounded-none border border-[var(--br-border-dim)] bg-transparent px-3 text-sm font-medium ring-offset-background transition-colors hover:bg-secondary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-muted/40 group-[.destructive]:hover:border-destructive/30 group-[.destructive]:hover:bg-destructive group-[.destructive]:hover:text-destructive-foreground group-[.destructive]:focus:ring-destructive',
        className
      )}
      {...props}
    />
...
const ToastTitle = React.forwardRef<React.ComponentRef<typeof ToastTitleBase>, React.ComponentPropsWithoutRef<typeof ToastTitleBase>>(
  ({ className, ...props }, ref) => (
    <ToastTitleBase
      ref={ref}
      className={cn('text-sm font-semibold font-mono uppercase tracking-tight', className)}
      {...props}
    />
  )
);
