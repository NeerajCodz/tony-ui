...
const EmptyState = React.forwardRef(({ className, type, title, description, action, icon, ...props }: any, ref: any) => {     
  const Icon = icon ? () => icon : styles.icon;
  const getTypeStyles = () => {
    if (type === 'inverse') return { backgroundColor: 'hsl(var(--primary-foreground))', color: 'hsl(var(--primary))' };
    if (type === 'contrast') return { border: '2px solid currentColor' };
    if (type === 'soft') return { backgroundColor: 'hsl(var(--muted))' };
    return {};
  };

  return (
    <div ref={ref} className={cn(styles.root, className)} style={getTypeStyles()} {...props}>
