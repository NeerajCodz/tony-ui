...
const Component = React.forwardRef<HTMLDivElement, DirectionProps>(({ direction = 'right', size = 'md', className, version = 'angular-corner', variant = 'primary', type = 'default', animated = true, children, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("relative flex items-center justify-center transition-all duration-300", sizeMap[size], animated && "hover:scale-110", className)} style={{ ...styles, clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)', color: `hsl(var(--${color}-foreground))` }} {...props}>
      <div className={cn("transition-transform duration-500", animated && "group-hover:rotate-45")}>
        {children || getIcon()}
      </div>
      <div className="absolute inset-0 pointer-events-none opacity-50" style={{ border: `1px dashed hsl(var(--${color}-base) / 0.3)`, clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }} />
    </div>
  )
})
Component.displayName = "Direction-angular-corner"
