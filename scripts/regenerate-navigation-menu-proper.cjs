...
export const NavigationMenu = ({ 
  ...
  const Component = useMemo(() => lazy(() => import(\`./navigation-menu-\${version}\`).then(module => ({ default: module.NavigationMenuRoot }))), [version]);
...
const createSubComponent = (name: string, exportName: string) => {
  return ({ children, ...props }: any) => {
    ...
    const Component = useMemo(() => lazy(() => import(\`./navigation-menu-\${version}\`).then(module => ({ default: module[exportName] }))), [version]);
...
export const NavigationMenuList = ({ className, ...props }: any) => {
  return (
    <NavigationMenuPrimitive.List
      className={cn(getListStyles('${version}'), className)}
      {...props}
    />
  );
};
  ...
  return (
    <div className={cn("absolute left-0 top-full flex justify-center")}>
      <NavigationMenuPrimitive.Viewport
        className={cn(getViewportStyles('${version}'), className)}
        {...props}
      />
    </div>
  );
...
`;
  ...
  fs.writeFileSync(path.join(componentDir, `navigation-menu-${version}.tsx`), content);
  console.log(`Generated navigation-menu-${version}.tsx`);
};

// Execute
generateIndex();
versions.forEach(v => generateVersion(v));
console.log('Navigation Menu regeneration complete.');
