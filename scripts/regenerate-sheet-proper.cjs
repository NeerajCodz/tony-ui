...
const versionComponents = {
  'angular-corner': lazy(() => import('./sheet-angular-corner')),
  'holo-frame': lazy(() => import('./sheet-holo-frame')),
  'data-panel': lazy(() => import('./sheet-data-panel')),
  'circuit-board': lazy(() => import('./sheet-circuit-board')),
  'quantum-gate': lazy(() => import('./sheet-quantum-gate')),
  'tactical-hud': lazy(() => import('./sheet-tactical-hud')),
  'energy-shield': lazy(() => import('./sheet-energy-shield')),
  'terminal-window': lazy(() => import('./sheet-terminal-window')),
  'matrix-grid': lazy(() => import('./sheet-matrix-grid')),
  'neon-outline': lazy(() => import('./sheet-neon-outline')),
};
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({ 
  ...
  return (
    <SheetPortal>
      {/* We need to use VersionModule's overlay if customized, but typically overlay is separate. 
          Actually, we should let the VersionModule handle Overlay + Content structure if needed, 
          but usually Overlay is sibling to Content in Portal.
          We will wrap Overlay in Suspense too or just render default overlay.
          For simplicity, we render a default overlay here or let the version file export it. 
          Let's render a default Overlay and pass styles if possible, but radix Portal expects children.
      */}
      <Suspense fallback={<SheetPrimitive.Content className={cn("fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500", className)} {...props}>{children}</SheetPrimitive.Content>}>
        {/* @ts-ignore */}
        <VersionModule.default ref={ref} side={side} className={className} {...props}>
          {children}
...
export {
  Sheet,
  SheetPortal,
  SheetOverlay: SheetPrimitive.Overlay,
  SheetTrigger,
  SheetClose,
  SheetContent,
  SheetHeader,
...
`;
  ...
  fs.writeFileSync(path.join(targetDir, `sheet-${version}.tsx`), versionContent);
});

console.log('Sheet regeneration complete.');
