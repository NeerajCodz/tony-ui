...
const AnalogClock = React.forwardRef<SVGSVGElement, AnalogClockProps>(
  ({ className, effects = 'on', size = 'md', visualType = 'outline', ...props }, ref) => {
    ...
    return (
      <svg
        ref={ref}
        className={cn(holoFrameEffectsClass(effects), 'bg-[var(--hf-surface)] text-[var(--hf-text)] border border-[var(--hf-border-dim)] rounded-full', sizeClasses[size], className)}
        viewBox="0 0 100 100"
        {...props}
      >
        <circle cx="50" cy="50" r="48" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--hf-border-dim)]" />
        ...
        <line
          ...
          className="text-[var(--hf-border-main)]"
        />
        
        <circle cx="50" cy="50" r="3" fill="currentColor" className="text-[var(--hf-border-main)]" />
      </svg>
    );
  }
);
