...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        'fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col border border-[var(--dp-border)] bg-[var(--dp-surface)] text-[var(--text-primary)] shadow-2xl',
        className
      )}
      style={{ clipPath: 'polygon(var(--corner) 0%, calc(100% - var(--corner)) 0%, 100% 100%, 0% 100%)', '--corner': '20px' } as React.CSSProperties}
      {...props}
    >
      <div className="mx-auto mt-4 h-2 w-[100px] rounded-full bg-[var(--dp-border)] opacity-50" />
      {children}
    </DrawerPrimitive.Content>
  </DrawerPortal>
));
