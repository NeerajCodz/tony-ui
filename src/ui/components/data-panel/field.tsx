  ...
  ({ className, size = 'md', ...props }, ref) => {
    ...
    return (
      <FieldLabelBase
        ref={ref}
        className={cn(
            'font-mono font-bold uppercase tracking-wider text-[var(--text-secondary)] data-[disabled]:opacity-70 group-focus-within/field:text-[var(--dp-accent)] transition-colors',
            sizeClass,
            className
        )}
        {...props}
...
export const FieldError = React.forwardRef<HTMLParagraphElement, FieldErrorProps>(
  ({ className, ...props }, ref) => {
    return (
      <FieldErrorBase
        ref={ref}
        className={cn('text-[0.8rem] font-medium text-[var(--dp-critical)] font-mono flex items-center gap-1 before:content-[">"] before:mr-1', className)}
        {...props}
      />
    );
  }
