...
const Slider = React.forwardRef<React.ElementRef<typeof SliderBase>, SliderProps>(({ type, className, variant = 'primary', ...props }, ref) => {
  ...
  return (
    <SliderBase
      ...
      {...props}
    >
      <SliderTrackBase className="relative h-2 w-full grow overflow-hidden rounded-full bg-secondary" style={{ backgroundColor: `hsl(var(--${color}-base) / 0.2)` }}>
        <SliderRangeBase className="absolute h-full bg-primary" style={{ backgroundColor: `hsl(var(--${color}-base))`, boxShadow: `0 0 8px hsl(var(--${color}-base))` }} />
      </SliderTrackBase>
      <SliderThumbBase
