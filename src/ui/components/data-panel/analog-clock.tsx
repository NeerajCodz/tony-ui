...
const AnalogClock = React.forwardRef<SVGSVGElement, AnalogClockProps>(
  ({ className, size = 'md', visualType = 'outline', ...props }, ref) => {
    ...
    return (
      <svg
        ref={ref}
        className={cn('bg-[var(--dp-surface)] text-[var(--text-primary)] border border-[var(--dp-border)] rounded-full', sizeClasses[size], className)}
        viewBox="0 0 100 100"
        {...props}
      >
        <circle cx="50" cy="50" r="48" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--dp-border)]" />
        ...
        <circle cx="50" cy="50" r="3" fill="currentColor" className="text-[var(--dp-accent)]" />
      </svg>
    );
  }
);
