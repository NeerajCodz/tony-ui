        ...
        <PopoverTrigger asChild>
          <button
            ...
            className={cn(quantumGateEffectsClass(effects), 
                'flex h-10 w-full items-center justify-between border border-[var(--qg-border)] bg-[var(--qg-surface)] px-3 py-2 text-sm placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--qg-iris-1)] disabled:cursor-not-allowed disabled:opacity-50 font-sans text-[var(--text-primary)] transition-colors hover:bg-[var(--qg-iris-1)]/10',
                className
            )}
                  ...
                  <CommandGroup>
                    {options.map((option) => (
                      ...
                      >
                        <Check
                          className={cn(quantumGateEffectsClass(effects), 
                            'mr-2 h-4 w-4 text-[var(--qg-iris-1)]',
...
export const ComboboxTrigger = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { effects?: QuantumGateEffects }>(
