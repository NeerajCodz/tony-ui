...
const TEMPLATE = (version) => {
  const isCustom = ['angular-corner', 'holo-frame', 'terminal-window'].includes(version);
  const defaultStyle = isCustom ? '' : `border: \`1px solid hsl(var(--\${color}-base) / 0.3)\`, backgroundColor: \`hsl(var(--\${color}-base) / 0.05)\``;
...
const DataTable = <TData, TValue>({
  ...
  // @ts-ignore
  const currentStyle = versionStyles['${version}'] || versionStyles['angular-corner'];
    ...
    <div className={cn("w-full overflow-hidden", className)} style={{
      ...
      <div 
        ...
        style={{
          ...
          ${version === 'angular-corner' ? `clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)', backgroundColor: \`hsl(var(--\${color}-base) / 0.05)\`, border: \`1px solid hsl(var(--\${color}-base) / 0.5)\`` : ''}
          ${version === 'holo-frame' ? `boxShadow: \`0 0 15px hsl(var(--\${color}-base) / 0.1)\`, border: \`1px solid hsl(var(--\${color}-base) / 0.3)\`, backgroundColor: \`hsl(var(--\${color}-base) / 0.02)\`` : ''}
          ${version === 'terminal-window' ? `border: \`2px solid hsl(var(--\${color}-base))\`, backgroundColor: 'black', fontFamily: 'monospace'` : ''}
...
${VERSIONS.map(v => `  '${v.name}': lazy(() => import('./data-table-${v.name}')),`).join('\n')}
};
...
VERSIONS.forEach(version => {
  const filePath = path.join(COMPONENT_DIR, `data-table-${version.name}.tsx`);
  fs.writeFileSync(filePath, TEMPLATE(version.name));
  console.log(`Generated data-table-${version.name}.tsx`);
});
...
console.log('DataTable regeneration complete.');
