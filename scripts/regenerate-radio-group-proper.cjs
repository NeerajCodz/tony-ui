...
// --- Context ---
const VersionContext = createContext<RadioGroupVersion>('angular-corner');

// --- Lazy Loaders ---
const versionComponents = {
  'angular-corner': lazy(() => import('./radio-group-angular-corner')),
  'holo-frame': lazy(() => import('./radio-group-holo-frame')),
  'data-panel': lazy(() => import('./radio-group-data-panel')),
  'circuit-board': lazy(() => import('./radio-group-circuit-board')),
  'quantum-gate': lazy(() => import('./radio-group-quantum-gate')),
  'tactical-hud': lazy(() => import('./radio-group-tactical-hud')),
  'energy-shield': lazy(() => import('./radio-group-energy-shield')),
  'terminal-window': lazy(() => import('./radio-group-terminal-window')),
  'matrix-grid': lazy(() => import('./radio-group-matrix-grid')),
  'neon-outline': lazy(() => import('./radio-group-neon-outline')),
};
...
const RadioGroup = React.forwardRef<React.ElementRef<typeof RadioGroupPrimitive.Root>, RadioGroupProps>(({ 
  ...
  return (
    <VersionContext.Provider value={version}>
      <Suspense fallback={<RadioGroupPrimitive.Root className={cn("grid gap-2", className)} {...props} />}>
        {/* @ts-ignore */}
...
const RadioGroupItem = React.forwardRef<React.ElementRef<typeof RadioGroupPrimitive.Item>, React.ComponentPropsWithoutRef<typeof RadioGroupPrimitive.Item>>(({ className, ...props }, ref) => {
  ...
  return (
     <Suspense fallback={<RadioGroupPrimitive.Item ref={ref} className={cn("aspect-square h-4 w-4 rounded-full border border-primary text-primary ring-offset-background focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50", className)} {...props}><RadioGroupPrimitive.Indicator className="flex items-center justify-center"><Circle className="h-2.5 w-2.5 fill-current text-current" /></RadioGroupPrimitive.Indicator></RadioGroupPrimitive.Item>}>
      {/* @ts-ignore */}
      <VersionModule.Item ref={ref} className={className} {...props} />
    </Suspense>
  )
})
...
const Icon = ['angular-corner', 'data-panel', 'tactical-hud', 'terminal-window'].includes('${version}') ? Square : Circle;
...
`;
  ...
  fs.writeFileSync(path.join(targetDir, `radio-group-${version}.tsx`), versionContent);
});

console.log('RadioGroup regeneration complete.');
