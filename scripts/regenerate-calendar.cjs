...
const COMPONENT_PATH = path.join(UI_PATH, 'components/calendar');

if (!fs.existsSync(COMPONENT_PATH)) {
  fs.mkdirSync(COMPONENT_PATH, { recursive: true });
}

// Calendar Clip Paths (Complex panel shapes)
const VERSIONS = {
  'angular-corner': {
  ...
  'circuit-board': {
    clip: 'polygon(10px 0, 50px 0, 60px 10px, 100% 10px, 100% calc(100% - 10px), calc(100% - 50px) 100%, 10px 100%, 0 calc(100% - 10px), 0 10px)',
    anim: 'glitch-in'
  },
  'quantum-gate': {
...
Component.displayName = 'Calendar-${version}';
...
Object.entries(VERSIONS).forEach(([name, config]) => {
  ...
  const filePath = path.join(COMPONENT_PATH, `calendar-${name}.tsx`);
  fs.writeFileSync(filePath, content);
  console.log(`Generated calendar-${name}.tsx`);
});

console.log('Calendar regeneration complete.');
