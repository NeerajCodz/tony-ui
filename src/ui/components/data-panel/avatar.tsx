  ...
  ({ className, size = 'md', shape = 'circle', style, ...props }, ref) => {
    return (
      <AvatarBase
        ...
        style={{ ...style }}
        className={cn(
          'relative flex shrink-0 overflow-hidden bg-[var(--dp-surface)] border border-[var(--dp-border)]',
          getSizeStyles(size),
          className
        )}
        {...props}
...
>(({ className, ...props }, ref) => (
  <AvatarFallbackBase
    ref={ref}
    className={cn(
      'flex h-full w-full items-center justify-center bg-[var(--dp-surface)] text-[var(--text-secondary)] font-mono font-bold',
      className
    )}
    {...props}
  />
