...
const CLIP_PATH = 'polygon(0 0, 100% 0, 100% calc(100% - 28px), calc(100% - 28px) 100%, 0 100%)';
...
const Component = React.forwardRef<HTMLDivElement, CardProps>(({
  ...
  return (
    ...
    >
      ...
      {/* Top accent bar */}
      <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: `linear-gradient(90deg, var(--${color}), rgba(var(--${color}-rgb), 0.2))` }} />
      {/* Bottom-right cut indicators */}
      <div className="absolute bottom-[28px] right-0 w-[3px] h-6" style={{ background: `var(--${color})` }} />
      <div className="absolute bottom-0 right-[28px] w-6 h-[3px]" style={{ background: `var(--${color})` }} />
...
Component.displayName = 'Card-tech-panel';
