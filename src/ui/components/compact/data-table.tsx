...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      border: '1px solid hsl(var(--border))', borderRadius: '0.25rem', fontSize: '0.8rem'
    , ...typeStyles}}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             background: 'hsl(var(--muted))', padding: '0.25rem'
          , ...typeStyles}}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: '1px solid hsl(var(--border))', padding: '0.25rem' , ...typeStyles}}
                >
                  {columns.map((col, j) => (
