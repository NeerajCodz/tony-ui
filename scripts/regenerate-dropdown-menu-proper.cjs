...
const createSubComponent = (name: string, exportName: string) => {
  return ({ children, ...props }: any) => {
    ...
    const Component = useMemo(() => lazy(() => import(\`./dropdown-menu-\${version}\`).then(module => ({ default: module[exportName] }))), [version]);
...
`;
  ...
  fs.writeFileSync(path.join(componentDir, `dropdown-menu-${version}.tsx`), content);
  console.log(`Generated dropdown-menu-${version}.tsx`);
};

// Execute
generateIndex();
versions.forEach(v => generateVersion(v));
console.log('Dropdown Menu regeneration complete.');
