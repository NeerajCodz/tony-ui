    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          style={{ ...styles, clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)', fontFamily: 'var(--font-mono)', padding: '0 1rem', color: `hsl(var(--${color}-foreground))` }}
          {...props}
        >
          <CalendarIcon className="mr-2 h-4 w-4 opacity-70" />
          {date ? format(date, "PPP") : <span>{placeholder}</span>}
          <div className="absolute right-0 top-0 h-2 w-2 border-t border-r border-[hsl(var(--primary-base)/0.5)]" /><div className="absolute left-0 bottom-0 h-2 w-2 border-b border-l border-[hsl(var(--primary-base)/0.5)]" />
        </button>
      </PopoverTrigger>
...
Component.displayName = "DatePicker-angular-corner"
