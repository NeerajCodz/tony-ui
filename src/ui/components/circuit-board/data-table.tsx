...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      backgroundImage: `radial-gradient(hsl(var(--${color}-base) / 0.2) 1px, transparent 1px)`, backgroundSize: '20px 20px', border: `1px solid hsl(var(--${color}-base) / 0.3)`
    , ...typeStyles}}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             background: `hsl(var(--${color}-base) / 0.1)`, borderBottom: `2px solid hsl(var(--${color}-base) / 0.4)`
          , ...typeStyles}}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: `1px solid hsl(var(--${color}-base) / 0.2)` , ...typeStyles}}
                >
                  {columns.map((col, j) => (
