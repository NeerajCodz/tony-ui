...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      border: `2px solid hsl(var(--${color}-base))`, borderRadius: '0.25rem', background: 'hsl(var(--background))'
    , ...typeStyles}}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             borderBottom: `2px solid hsl(var(--${color}-base))`
          , ...typeStyles}}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: `1px solid hsl(var(--${color}-base))` , ...typeStyles}}
                >
                  {columns.map((col, j) => (
