...
const AnalogClock = React.forwardRef<SVGSVGElement, AnalogClockProps>(
  ({ className, effects = 'on', size = 'md', visualType = 'outline', ...props }, ref) => {
    ...
    return (
      <svg
        ref={ref}
        className={cn(techPanelEffectsClass(effects), 'bg-[var(--tp-panel)] text-[var(--text-primary)] border border-[var(--tp-border-outer)] rounded-none', sizeClasses[size], className)}
        viewBox="0 0 100 100"
        {...props}
      >
        <circle cx="50" cy="50" r="48" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--tp-border-inner)]" />
        ...
        <circle cx="50" cy="50" r="3" fill="currentColor" className="text-[var(--tp-accent)]" />
      </svg>
    );
  }
);
