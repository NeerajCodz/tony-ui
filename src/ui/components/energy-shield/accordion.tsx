...
const Accordion = React.forwardRef<React.ComponentRef<typeof AccordionPrimitive.Root>, EnergyShieldAccordionProps>(
  ({ className, effects = 'on', ...props }, ref) => (
    <AccordionBase 
      ref={ref} 
      className={cn(energyShieldEffectsClass(effects), 'space-y-2', className)} 
      {...props} 
    />
  )
);
...
export type EnergyShieldAccordionProps = AccordionBaseProps & {
  ...
  ({ className, effects = 'on', style, ...props }, ref) => (
    <AccordionItemBase
      ...
      className={cn(energyShieldEffectsClass(effects), 
        'border border-[var(--es-hex-line)] bg-[var(--es-surface)]  transition-colors',
        'data-[state=open]:border-[var(--es-plasma-1)]',
        className
      )}
      {...props}
    />
  ...
  ({ className, effects = 'on', children, ...props }, ref) => (
    <AccordionHeaderBase className="flex">
      <AccordionTriggerBase
        ref={ref}
        className={cn(energyShieldEffectsClass(effects), 
          'flex flex-1 items-center justify-between px-4 py-4 font-sans font-bold uppercase tracking-wider transition-all hover:text-[var(--es-plasma-1)] [&[data-state=open]>svg]:rotate-180',
          className
        )}
        {...props}
      >
