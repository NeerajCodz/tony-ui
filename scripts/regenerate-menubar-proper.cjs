...
export const Menubar = ({ 
  ...
  const Component = useMemo(() => lazy(() => import(\`./menubar-\${version}\`).then(module => ({ default: module.MenubarRoot }))), [version]);
...
const createSubComponent = (name: string, exportName: string) => {
  return ({ children, ...props }: any) => {
    ...
    const Component = useMemo(() => lazy(() => import(\`./menubar-\${version}\`).then(module => ({ default: module[exportName] }))), [version]);
...
`;
  ...
  fs.writeFileSync(path.join(componentDir, `menubar-${version}.tsx`), content);
  console.log(`Generated menubar-${version}.tsx`);
};
