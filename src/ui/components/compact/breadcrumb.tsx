...
const Breadcrumb = React.forwardRef<HTMLElement, BreadcrumbProps>(
  ({ className, ...props }, ref) => (
    <BreadcrumbBase ref={ref} className={cn('font-mono  tracking-normal', className)} {...props} />
  )
);
...
const BreadcrumbLink = React.forwardRef<HTMLAnchorElement, React.ComponentPropsWithoutRef<typeof BreadcrumbLinkBase>>(
  ({ className, ...props }, ref) => (
    <BreadcrumbLinkBase
      ref={ref}
      className={cn('transition-colors hover:text-[var(--cp-accent)] hover:underline', className)}
      {...props}
    />
  )
);
...
const BreadcrumbSeparator = React.forwardRef<HTMLLIElement, React.ComponentPropsWithoutRef<typeof BreadcrumbSeparatorBase>>(
  ({ children, className, ...props }, ref) => (
    <BreadcrumbSeparatorBase
      ref={ref}
      className={cn('text-[var(--cp-accent)]/50', className)}
      {...props}
    >
