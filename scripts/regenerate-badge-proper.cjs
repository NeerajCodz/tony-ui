...
const versionComponents = {
  'angular-corner': lazy(() => import('./badge-angular-corner')),
  'holo-frame': lazy(() => import('./badge-holo-frame')),
  'data-panel': lazy(() => import('./badge-data-panel')),
  'circuit-board': lazy(() => import('./badge-circuit-board')),
  'quantum-gate': lazy(() => import('./badge-quantum-gate')),
  'tactical-hud': lazy(() => import('./badge-tactical-hud')),
  'energy-shield': lazy(() => import('./badge-energy-shield')),
  'terminal-window': lazy(() => import('./badge-terminal-window')),
  'matrix-grid': lazy(() => import('./badge-matrix-grid')),
  'neon-outline': lazy(() => import('./badge-neon-outline')),
};
...
const Badge = React.forwardRef<HTMLDivElement, BadgeProps>(({ 
  ...
  return (
    <Suspense fallback={<div className={cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2", className)} {...props} />}>
      {/* @ts-ignore */}
...
`;
  ...
  fs.writeFileSync(path.join(targetDir, `badge-${version}.tsx`), versionContent);
});

console.log('Badge regeneration complete.');
