...
const DataTable = <TData, TValue>({
  ...
  return (
    <div className={cn("w-full overflow-hidden", className)} style={{
      border: `1px solid hsl(var(--${color}-base))`, background: 'hsl(var(--background) / 0.9)', position: 'relative'
    , ...typeStyles}}>
      <div 
      ...
      >
        <table className="w-full caption-bottom text-sm">
          <thead style={{
             background: `hsl(var(--${color}-base) / 0.2)`, textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 'bold'
          , ...typeStyles}}>
            ...
            {sortedData.length > 0 ? (
              sortedData.map((row, i) => (
                <tr 
                  ...
                  style={{ borderBottom: `1px solid hsl(var(--${color}-base) / 0.3)` , ...typeStyles}}
                >
                  {columns.map((col, j) => (
