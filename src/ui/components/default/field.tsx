...
export const FieldLabel = React.forwardRef<HTMLLabelElement, FieldLabelProps>(
  ({ className, size = 'md', ...props }, ref) => {
    ...
    return (
      <FieldLabelBase
        ref={ref}
        className={cn('font-medium leading-none text-[var(--df-text)] data-[disabled]:opacity-70', sizeClass, className)}
        {...props}
      />
    );
  }
...
export const FieldDescription = React.forwardRef<HTMLParagraphElement, FieldDescriptionProps>(
  ({ className, ...props }, ref) => {
    return (
      <FieldDescriptionBase
        ref={ref}
        className={cn('text-[0.8rem] text-[var(--df-muted)]', className)}
        {...props}
      />
    );
  }
