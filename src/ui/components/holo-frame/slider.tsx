...
const Slider = React.forwardRef<React.ElementRef<typeof SliderPrimitive.Root>, SliderProps>(({ type, className, variant = 'primary', ...props }, ref) => {
  ...
  return (
    <SliderPrimitive.Root
      ...
      {...props}
    >
      <SliderPrimitive.Track className="relative h-2 w-full grow overflow-hidden rounded-full bg-secondary" style={{ backgroundColor: `hsl(var(--${color}-base) / 0.2)` }}>
        <SliderPrimitive.Range className="absolute h-full bg-primary" style={{ backgroundColor: `hsl(var(--${color}-base))`, boxShadow: `0 0 8px hsl(var(--${color}-base))` }} />
      </SliderPrimitive.Track>
      <SliderPrimitive.Thumb
        className="block h-5 w-5 rounded-full border-2 border-primary bg-background ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
        style={{ borderColor: `hsl(var(--${color}-base))`, backgroundColor: `hsl(var(--${color}-base) / 0.2)`, boxShadow: `0 0 5px hsl(var(--${color}-base))` }}
      />
    </SliderPrimitive.Root>
  )
})
