...
const createSubComponent = (name: string, exportName: string) => {
  return ({ children, ...props }: any) => {
    ...
    const Component = useMemo(() => lazy(() => import(\`./context-menu-\${version}\`).then(module => ({ default: module[exportName] }))), [version]);
...
`;
  ...
  fs.writeFileSync(path.join(componentDir, `context-menu-${version}.tsx`), content);
  console.log(`Generated context-menu-${version}.tsx`);
};

// Execute
generateIndex();
versions.forEach(v => generateVersion(v));
console.log('Context Menu regeneration complete.');
