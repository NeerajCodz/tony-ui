...
export const Carousel = ({ 
  ...
  const Component = useMemo(() => lazy(() => import(\`./carousel-\${version}\`).then(module => ({ default: module.CarouselRoot }))), [version]);

  return (
    <VersionContext.Provider value={version}>
      <Suspense fallback={<Skeleton className="h-[200px] w-full opacity-20" />}>
...
const createSubComponent = (name: string, exportName: string) => {
  return ({ children, ...props }: any) => {
    ...
    const Component = useMemo(() => lazy(() => import(\`./carousel-\${version}\`).then(module => ({ default: module[exportName] }))), [version]);

    return (
      <Suspense fallback={name === 'Content' ? <div className="flex gap-4 overflow-hidden">{children}</div> : null}>
        <Component {...props}>{children}</Component>
      </Suspense>
    );
  };
...
Carousel.Content = createSubComponent('Content', 'CarouselContent');
Carousel.Item = createSubComponent('Item', 'CarouselItem');
Carousel.Previous = createSubComponent('Previous', 'CarouselPrevious');
Carousel.Next = createSubComponent('Next', 'CarouselNext');
...
const generateVersion = (version) => {
  const content = `import React, { useCallback, useEffect, useState, createContext, useContext } from 'react';
    ...
    return (
      ...
      >
        <div
          ...
          className={cn(getRootStyles('${version}'), className)}
          role="region"
...
`;
  ...
  fs.writeFileSync(path.join(componentDir, `carousel-${version}.tsx`), content);
  console.log(`Generated carousel-${version}.tsx`);
};
